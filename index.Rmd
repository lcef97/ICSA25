...
```{r Regions, echo = FALSE, warnings = FALSE}
suppressWarnings(Regions <- SchoolDataIT:::prov.names() %>% 
  dplyr::mutate(Region_initials = dplyr::case_when(
    .data$Region_code ==  1 ~ "PIE",
    .data$Region_code ==  2 ~ "VDA",
    .data$Region_code ==  3 ~ "LOM",
    .data$Region_code ==  4 ~ "TAA",
    .data$Region_code ==  5 ~ "VEN",
    .data$Region_code ==  6 ~ "FVG",
    .data$Region_code ==  7 ~ "LIG",
    .data$Region_code ==  8 ~ "ERM",
    .data$Region_code ==  9 ~ "TOS",
    .data$Region_code == 10 ~ "UMB",
    .data$Region_code == 11 ~ "MAR",
    .data$Region_code == 12 ~ "LAZ",
    .data$Region_code == 13 ~ "ABR",
    .data$Region_code == 14 ~ "MOL",
    .data$Region_code == 15 ~ "CAM",
    .data$Region_code == 16 ~ "PUG",
    .data$Region_code == 17 ~ "BAS",
    .data$Region_code == 18 ~ "CAL",
    .data$Region_code == 19 ~ "SIC",
    .data$Region_code == 20 ~ "SAR"
  )))
...
- **Scope** of this line of research:
  - Foster a **Multidimensional** analysis of the state of the school system
...
- version <a href="https://cran.r-project.org/web/packages/SchoolDataIT/index.html" target="_blank"> 0.2.2 </a> on CRAN 

- Constant **maintenance** <a href="https://github.com/lcef97/SchoolDataIT/tree/main" target="_blank"> on Github </a> (version 0.2.3, to be submitted to CRAN)
...
package.scheme.1 <- function(width = 900) {
  return(DiagrammeR::grViz("digraph {
    ...
    node [shape = parallelogram, style = filled, fillcolor = \"#004E5A60\", fontcolor = black, fontname = 'Poppins', color = \"#004E5A\", fontsize = 20 ]
    zero[label = 'User-defined input']
    end1[label = 'Merged database']
    end2[label = 'Maps']

    node [shape = rectangle, style = filled, fillcolor = \"#004E5A60\", fontcolor = black, fontname = 'Poppins', color = \"#004E5A\", fontsize = 20, width = 3.4]
    ...
    node [shape = oval, style = filled, fillcolor = \"#004E5A60\", fontcolor = black, fontname = 'Poppins', color = \"#004E5A\", fontsize = 20, width = 3.4]
...
- Scope: **last** year of **middle** school, year **2022/23** (last one with complete observations)
...
- Students outcome measured by **Invalsi scores** (spatially homogeneous indicator, high-quality datum)
  ...
  - Available for $\approx 2000$ municipalities
...
- Simplest analysis: **OLS** regression of **Invalsi scores** on average **class size** (municipality-level data)
...
- Does classroom size depend on some **other variables** we are overlooking? &rarr; distribution across the $6$ classes of the inner areas taxonomy (**A**, **B**: pole; **C**: belt; **D**: intermediate; **E**: peripheral; **F**: ultra-peripheral) 


```{r nstud boxplot, message = FALSE, warning=FALSE, echo = FALSE, fig.height=3}
...
- $u$ defined over a **continuous spatial domain** with prior distribution: $u \sim N(0, \Sigma)$ &rarr; $\Sigma$: Matérn correlation matrix <div class = "footnote"> see:  Banerjee, S., Carlin, B.P., Gelfand, A.E. (2014). *Hierarchical Modeling and Analysis for Spatial Data*, 2nd edition, Chapman and Hall/CRC </div>
  
- $u$ is approximated with a **Gaussian Markov Random Field** defined on a **2-d manifold** <div class = "footnote">
...
- Model estimated using the **Integrated Nested Laplace Approximation** (**INLA**) <div class = "footnote">
...
- North-South trend  the pole/periphery differences: already kept into account 

- Hyperparameters posterior summary: median $\sigma^2$ (scale parameter of the spatial effect) about $31$ points
...
- **Darkest** regions: education **vulnerability** not captured by covariates
  - Po Delta; Gargano peninsula; Neaples hinterland; upper Ionian Calabria; Northwestern Sardinia
...
- Assessing the impact of **class size** on student outcomes is less simple than it seems 
- **Auxiliary** information such as the **degree of centrality** of municipalities or **geographical information** becomes crucial
